...
**
**	DESCRIPTION
**	    Les fonctions de libfl.a sont regroupees en trois categories:
**
**	    - les IO standard (open, read, write, etc...)
**	    - les IO structurees
**	    - les fonctions de la toolbox
**
**	    1. IO standard
**
**	    Le but des fonctions d'IO bas niveau de libfl est de presenter
**	    les fichiers de maniere identique quelque soit leur mode d'IO
**	    effectif (fichier disque mappe ou non, pipe, socket, zone
**	    memoire, etc...). Le nom de fichier passe a la fonction FLopen
**	    permet de definir des objets plus complexes que de simples
**	    fichiers disque (par exemple, "mmap:file" ou "pipe:cat file",
**	    cf FLopen pour plus de details).
**	    L'acces au fichier ainsi ouverts se fait de maniere uniforme par
**	    FLread et/ou FLwrite. Outre l'avantage d'un acces uniformise,
**	    ces fonctions levent egalement certaines impossibilites dues au
**	    mode d'ouverture des fichiers. Par exemple, l'ecriture dans un
**	    fichier pipe ouvert en lecture (type "pipe:cat file") est rendue
**	    possible.
**
**	    Les routines d'IO de ce niveau peuvent etre utilisees en
**	    remplacement les routines de libc (open, fopen, read, fread,
**	    write, fwrite...). Des fonctions permettent de gerer les path
**	    d'acces aux fichiers.
**
**	    2. IO structurees
**
**	    Les fichiers structures geres par les routines de libfl sont
**	    organises en blocs definis par un id et une taille de bloc tous
**	    deux codes sur 4 octets. Cette structure est celle du format IFF
**	    (Interchange File Format) utilise sur PC, Mac, Indigo... avec
**	    quelques extensions.
**	    Les ids sont divises en deux types: les ids definissant la
**	    structure du fichier et ceux contenant les donnees.
**	    La premiere categorie contient 4 identificateurs:
**
**	    FORM <taille> <type>: marque le debut d'un ensemble de blocs de
**		donnees, comparable au struct du C.
**	    CAT  <taille> <type>: defini un ensemble non ordonne de FORM 
**		par exemple, une librairie d'images ou de sons
**	    LIST <taille> <type>: defini un ensemble ordonne de FORM 
**		par exemple, une sequence d'images
**	    PROP <taille> <type>: marque le debut d'un ensemble de blocs
**		contenant des proprietes communes aux FORM d'un LIST
**		ces proprietes peuvent etre localement redefinies
**		commes les variables locales du C.
**		Un exemple d'utilisation du PROP pourrait etre la
**		definition de la taille et du nombre de plans d'une
**		sequence d'image.
**	
**	    Les blocs de donnes sont definis par:
**
**	    <id> <taille> [data]
**
**	    Exemple: une image pourrait avoir la structure suivante
**	
**	    FORM 12304 IMAG
**	      IHDR 200	... Header de l'image, taille, plans ...
**	      LINE 800	... donnees de la ligne 1 ...
**	      LINE 800      ... donnees de la ligne 2 ...
**	      ...
**
**	    pour une librairie,
**
...
**	        ...
**
**	    et pour une sequence,
**
**	    LIST 64200 IMAG
**	      PROP 208 IMAG
**	        IHDR 200		... Header commun ...
**	      FORM 12394 IMAG
**	        ...
**	      FORM 12304 IMAG
**	        IHDR 200		... Redefinition locale ...
**	        ...
** 
**	    Dans les differentes pages de doc des fonctions, les blocs de
**	    structure sont appeles "groups", les blocs de donnees "chunks".
**	    Les IO sur les chunks se font via les fonctions FLget/FLput et 
**	    leurs derivees. Les ouvertures/fermetures de groupes font appels
**	    a des fonctions specifiques (FLbgnrgroup, FLendrgroup...).
**	
**	    Notes sur la structure:
**
**	    Les donnees sont toutes ecrites au format Big-Endian (poid fort
**	    en premier) a l'exception des ids, consideres comme chaines de
**	    caracteres (le byte-swapping est gere a la compilation).
**	    La presence de la taille des blocs permet a un parser de sauter
**	    les infos qu'il ne reconnait pas. 
**	    Les groupes et les blocs de donnees sont alignes sur deux octets
**	    L'octet de "padding" eventuel n'est pas compte dans la taille
**	    suivant l'id.
**	    Des ids supplementaires sont definis pour des groupes alignes
**	    sur 4 ou 8 octets: FOR4, FOR8, CAT4, CAT8, LIS4, LIS8.
**	    Les blocs de donnees heritent de l'alignement du groupe qui les
**	    contient (de meme que les sous-groupes eventuels; il est donc
**	    illegal de creer un groupe aligne sur 2 octets a l'interieur
**	    d'un groupe aligne sur 4, l'inverse est cependant parfaitement
**	    valide).
**
**	    3. Toolbox
**
**	    La librairie libfl fournit egalement des outils pour la gestion
**	    des listes chainees (FLxxxnode et FLxxxlist), des buffers
**	    (FLmalloc et al) et des filtres externes (FLfilter et FLexec).
**
**	    Gestion des erreurs
**
**	    Les fonctions d'IO sont implementees de maniere a eviter les
**	    effets de bord en cas d'erreur. Toutefois, il est tres fortement
**	    conseille de ne pas "insister" et tenter de poursuivre la
**	    lecture/ecriture en cas d'erreur. 
**
**	    Pour plus de details sur les fonctions de libfl.a, se referer
**	    aux man pages et aux exemples fournis avec la librairie.
*/
...
__inline void FLswapint64(__int64 fSrc, __int64 *pfDst)
{
	__int64 result;
	__int64 tmp = *(__int64*) &fSrc;
	result = (        ((tmp & 0x00000000000000ff) << 56) |
...
__inline void FLswapint64(__int64 dSrc, __int64 *pdDst)
{
	_asm	mov eax, dword ptr [dSrc]
	_asm	ror ax, 8
	_asm	ror eax, 16
...
{
	asm volatile(
		"movl (%1), %%eax; bswap %%eax; movl %%eax, 4(%0); "
		"movl 4(%1), %%eax; bswap %%eax; movl %%eax, (%0);"
		: : "c" ((unsigned int *)pdDst), "d" ((unsigned int *)&dSrc)
		: "%eax" );
}

static inline void FLswapint64(__uint64_t dSrc, __uint64_t *pdDst)
{
	asm volatile(
		"movl (%1), %%eax; bswap %%eax; movl %%eax, 4(%0); "
		"movl 4(%1), %%eax; bswap %%eax; movl %%eax, (%0);"
