...
OutputIterator RE_CALL _reg_format_aux(OutputIterator out, 
                           ...
                           )
{
   ...
   while(*fmt)
   {
      ...
      case syntax_open_bracket:
         ++fmt;  // recurse
         oi_assign(&out, _reg_format_aux(out, m, fmt, false MAYBE_PASS_LOCALE(l)));
         continue;
      ...
      case syntax_question:
      {
         ...
         if(m[id].matched)
         {
            oi_assign(&out, _reg_format_aux(out, m, fmt, true MAYBE_PASS_LOCALE(l)));
...
OutputIterator RE_CALL reg_format(OutputIterator out,
                          ...
                          const charT* fmt
#ifdef RE_LOCALE_CPP
                         , JM_STD::locale locale_inst = JM_STD::locale()
#endif
                         )
{
   //
...
#endif
   return _reg_format_aux(out, m, fmt, false MAYBE_PASS_LOCALE(locale_inst));
}
...
public:
   ...
   string_out_iterator<S>& operator++() { return *this; }
   string_out_iterator<S>& operator++(int) { return *this; }
   string_out_iterator<S>& operator*() { return *this; }
   string_out_iterator<S>& operator=(typename S::value_type v) 
   { 
      out->append(1, v); 
      return *this; 
   }
...
JM_STD::basic_string<charT> RE_CALL reg_format(const reg_match<iterator, Allocator>& m, const charT* fmt
#ifdef RE_LOCALE_CPP
                                  , JM_STD::locale locale_inst = JM_STD::locale()
#endif
                                  )
{
   JM_STD::basic_string<charT> result;
   string_out_iterator<JM_STD::basic_string<charT> > i(result);
   reg_format(i, m, fmt MAYBE_PASS_LOCALE(locale_inst));
   return result;
}
#elif !defined(JM_NO_STRING_H)
template <class iterator, class Allocator>
JM_STD::string RE_CALL reg_format(const reg_match<iterator, Allocator>& m, const char* fmt
#ifdef RE_LOCALE_CPP
                                  , JM_STD::locale locale_inst = JM_STD::locale()
#endif
                                  )
...
public:
   ...
   ~merge_out_predicate() {}
   bool RE_CALL operator()(const JM::reg_match<iterator, Allocator>& m)
   {
      const charT* f = fmt;
      if(copy_none)
         oi_assign(out, re_copy_out(*out, iterator(m[-1].first), iterator(m[-1].second)));
      oi_assign(out, _reg_format_aux(*out, m, f, false MAYBE_PASS_LOCALE(l)));
      *last = m[-2].first;
      return true;
   }
};
...
#endif
{
   ...
   locale_initialiser.update();
#else
   JM_STD::locale locale_inst(e.locale());
   if(JM_HAS_FACET(locale_inst, regfacet<charT>) == false)
   {
#ifdef JM_MSC_VER
      locale_inst = JM_STD::_ADDFAC(locale_inst, new regfacet<charT>());
...
JM_STD::basic_string<charT> RE_CALL reg_merge(const JM_STD::basic_string<charT>& s,
                         const reg_expression<charT, traits, Allocator>& e, 
                         const charT* fmt, 
                         bool copy = true, 
                         unsigned int flags = match_default)
...
JM_STD::string RE_CALL reg_merge(const JM_STD::string& s,
